...
class AccountInvoiceImportConfig(models.Model):
    ...
    def _compute_download_config_count(self):
        rg_res = self.env['account.invoice.download.config'].read_group([('import_config_id', 'in', self.ids)], ['import_config_id'], ['import_config_id'])
        mapped_data = dict([(x['import_config_id'][0], x['import_config_id_count']) for x in rg_res])
        for config in self:
