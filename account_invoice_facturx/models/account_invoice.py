...
class AccountInvoice(models.Model):
    ...
    def _cii_add_date(
        ...
        # 102 = format YYYYMMDD
        if isinstance(date_datetime, str):
            date_datetime = fields.Date.from_string(date_datetime)
    ...
    def _cii_add_invoice_line_block(
        ...
        if iline.invoice_line_tax_ids:
            ...
            raise UserError(_(
                "At least one tax is missing on invoice line '%s'. "
                "Please add at least one tax to this invoice line.")
                % iline.name)
    ...
    def regular_pdf_invoice_to_facturx_invoice(self, pdf_content):
        ...
        if self.type in ('out_invoice', 'out_refund'):
            ...
            pdf_content = generate_facturx_from_binary(
                pdf_content, facturx_xml_str,
                facturx_level=level, check_xsd=False, pdf_metadata=pdf_metadata)
