...
``invoice2data.extract_data``. Disk templates remain authoritative; DB
templates are merged in alongside them, so a partner can ship a base set
on disk and override per-deployment in the UI.
"""
...
class AccountInvoiceImport(models.TransientModel):
    ...
    def invoice2data_parse_invoice(self, file_data, company):
        ...
        Mirrors the upstream method but replaces the explicit disk-only
        template build with disk + DB (via ``_invoice2data_collect_templates``).
        Keeps the rest of the flow (OCR fallback via tesseract, then
        ``invoice2data_to_parsed_inv``) untouched.
        """
    ...
    def _invoice2data_collect_templates(self):
        ...
        Override this hook in a downstream module to drop the disk set,
        filter templates, or stamp in environment-specific ones.
        """
        templates = []
