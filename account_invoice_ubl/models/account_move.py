...
class AccountMove(models.Model):
    ...
    def _ubl_add_attachments(self, parent_node, ns, version='2.1'):
        self.ensure_one()
        if self.company_id.embed_pdf_in_ubl_xml_invoice and not self.env.context.get('no_embedded_pdf'):
    ...
    def _ubl_add_tax_total(self, xml_root, ns, version='2.1'):
        ...
        if not float_is_zero(self.amount_tax, precision_digits=prec):
            ...
            for line in tax_lines:
                res.setdefault(line.tax_line_id.tax_group_id, {'base': 0.0, 'amount': 0.0, 'tax': False})
            ...
            for group, amounts in res:
                self._ubl_add_tax_subtotal(
                    amounts['base'], amounts['amount'], amounts['tax'], cur_name,
    ...
    def generate_invoice_ubl_xml_etree(self, version='2.1'):
        ...
        self._ubl_add_payment_means(
            self.invoice_partner_bank_id, self.payment_mode_id, self.invoice_date_due,
    ...
    def attach_ubl_xml_file_button(self):
        ...
        attach = self.env['ir.attachment'].with_context({}).create({
            'name': filename,
            'res_id': self.id,
