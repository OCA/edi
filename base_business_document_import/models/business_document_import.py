...
class BusinessDocumentImport(models.AbstractModel):
    ...
    def _match_partner_bank(
        ...
        try:
            ...
            chatter_msg.append(
                _("IBAN <b>%(iban)s</b> is not valid, so it has been ignored.")
                % {"iban": iban}
            )
            return False
        ...
        if bankaccount:
            ...
            chatter_msg.append(
                _(
                    ...
                    "<a href=# data-oe-model=res.partner data-oe-id=%(i)d>%(n)s</a>"
                )
    ...
    def _match_uom(self, uom_dict, chatter_msg, product=False):
        ...
        chatter_msg.append(
            _(
                ...
                "<ul><li>UNECE code: %(unece)s</li>"
                "<li>Name of the unit of measure: %(dict)s</li></ul>"
    ...
    def post_create_or_update(self, parsed_dict, record, doc_filename=None):
        ...
        if parsed_dict.get("note"):
            if doc_filename:
                msg = _("<b>Notes in file %(filename)s:</b>") % {
