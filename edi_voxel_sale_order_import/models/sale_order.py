...
class Company(models.Model):
    ...
    def _parse_general_data_voxel(self, vals, xml_root, error_msgs):
        ...
        if general_elements:
            ...
            if date_order:
                vals.update(
                    date_order=datetime.strptime(date_order,
                                                 "%Y-%m-%dT%H:%M:%S"),
                )
            # add requested_date
            ...
            if end_date:
                # add validity_date
                vals['validity_date'] = datetime.strptime(
                    end_date, "%Y-%m-%d").date()
    ...
    def _parse_supplier_data_voxel(self, vals, xml_root, error_msgs):
        ...
        if supplier_elements:
            ...
            if company:
                ...
                error_msgs.append(
                    _("Couldn't find any <b>Company</b> corresponding to "
        ...
        if customer_elements:
            ...
            if partner:
                ...
                msg_fields = self._get_voxel_msg_fields(
                    'res.partner', partner_data)
                error_msgs.append(
                    _("Couldn't find any <b>Delivery Address</b> "
    ...
    def _parse_partner_data_voxel(self, data, raise_error=True):
        ...
        if data.get('country_id'):
            domains.append(
                [('country_id.code_alpha3', '=', data['country_id'])])
    ...
    def _parse_taxes_product_voxel(self, line_vals, line_element, error_msgs):
        ...
        for tax_line_element in tax_line_elements:
            ...
            if tax:
                ...
                error_msgs.append(
                    _("Couldn't find any <b>Tax</b> corresponding to "
                      "the following information extracted from the Voxel "
                      "document:<br/>"
                      "<ul>%s</ul>") % (msg_fields))
    ...
    def _get_voxel_msg_fields(self, model_name, partner_data):
        ...
        for key, value in partner_data.items():
            if partner_data.get(key):
                field_str = model._fields[key].get_description(
                    self.env)["string"]
                msg_fields += "<li>%s: %s</li>" % (field_str, value)
        return msg_fields
