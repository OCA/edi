...
class SaleOrderImport(models.TransientModel):
    ...
    def _create_missing_partner_vals_cleanup(self, vals: dict) -> dict:
        ...
        # If the module ``base_address_extended`` is installed, Odoo will call
        # ``res.partner._inverse_street_data()`` after the partner is created, because
        # that method is declared as inverse method of ``street_number``, and it will
        # recompute the ``street`` field value base on ``street_[name|number|number2]``.
        # If ``street_name`` is missing at this point, the ``street`` field will simply
